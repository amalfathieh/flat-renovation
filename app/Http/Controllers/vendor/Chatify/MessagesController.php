...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            ...
            $customer_id = $customer->customerProfile->id;   // null-safe operator
            $employee_id = $user->employee->id;
            $conversation = Conversation::where('employee_id', $employee_id)
                ->where('customer_id', $customer_id)
                ->first(); // أو firstOrCreate([...])

            DB::transaction(function () use ($conversation, $user, $request,$attachment,$attachment_title) {
//                $message = Chatify::newMessage([
//                    'from_id' => Auth::user()->id,
//                    'to_id' => $request['id'],
//                    'body' => htmlentities(trim($request['message']), ENT_QUOTES, 'UTF-8'),
...
//                ]);
                \App\Models\Message::create([
                    ...
                    'sender_image'    => $user->employee->image,
                ]);
            });
...
//        dd($query);
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = User::where('id','!=',Auth::user()->id)
           ->whereHas('customerProfile.conversation')
...
//        $users = User::whereHas('customerProfile.conversations', function ($q) use ($employee) {
...
//        dd($users)
        $records = User::where('id','!=',Auth::user()->id)
                    ->where('name', 'LIKE', "%{$input}%")->whereHas('customerProfile.conversation')
                    ->paginate($request->per_page ?? $this->perPage);
