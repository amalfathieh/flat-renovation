...
class CodeController extends Controller
{
    ...
    public function verifyAccount(Request $request)
    {
        try {
            ...
            if (!$ver_code) {
                return Response::Error(('strings.invalid_code'), 404);
            }
            // find user's email
            ...
            return Response::Success($data, ('strings.created_successfully'));
        } catch (\Exception $ex) {
    ...
    public function sendCodeVerification(Request $request)
    {
        try {
            ...
            if (!$user) {
                return Response::Error(('strings.email_account_not_exist'));
            }

            //Send verification code to user
            $this->sendVerificationCode($user);

            return Response::Success(null, ('strings.code_sent_email'));
        } catch (\Exception $ex) {
    ...
    public function checkCode(Request $request)
    {
        try {
            ...
            return Response::Success(null, ('strings.code_is_correct'));
        } catch (\Exception $ex) {
            return Response::Error($ex->getMessage());
        }
    }
